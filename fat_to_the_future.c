             ...
             off_t offset, struct fuse_file_info *fi)
{
    ...
    size_t starting_block;
    metadata file_metadata;
    char seed_read[block_size + 2];
    char unencoded_read[2 * block_size + 2];
    int write_int;
    int read_int;
    int check = 0;
    ...
    memset(seed_read, '\0', (block_size + 2) * sizeof(char));
    memset(unencoded_read, '\0', (2 * block_size + 2) * sizeof(char));
    ...
    fseek(disk, block_size * current_block, SEEK_SET);
    fread(seed_read + 2, block_size, 1, disk);
    printf("Read seed from disk: %s\n", seed_read);

    // This will tell the python program that an empty string was sent on purpose
    if (strcmp("", seed_read + 2) == 0) {
        seed_read[1] == '1';
    }
